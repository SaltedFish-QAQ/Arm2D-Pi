...
static int16_t s_iFlipHourglassWaistHalfWidth;
static int16_t s_iFlipHourglassMaximumHalfWidth;
static uint32_t s_wFlipHourglassProfileScaleQ16;
...
static bool __flip_get_hourglass_span(int16_t iY,
                                      int16_t *piLeft,
                                      int16_t *piRight)
{
    int16_t iOuterLeft = s_tFlipContainer.tLocation.iX;
    int16_t iTop = s_tFlipContainer.tLocation.iY;
    int16_t iOuterRight = iOuterLeft
                        + s_tFlipContainer.tSize.iWidth - 1;
    ...
    *piRight = iOuterRight;
    return true;
#else
    int32_t iDistanceY = 2 * iY - (iTop + iBottom);

    if (iDistanceY < 0) {
        iDistanceY = -iDistanceY;
    }

    uint32_t wProfilePositionQ16 =
        (uint32_t)iDistanceY * s_wFlipHourglassProfileScaleQ16;
    uint8_t chProfile = flip_hourglass_profile_sample(
                            wProfilePositionQ16);
    uint32_t wProfileQ16 = (uint32_t)chProfile * 257u;
    int32_t iAllowedHalfWidth = s_iFlipHourglassWaistHalfWidth
        + (int32_t)(((uint32_t)(s_iFlipHourglassMaximumHalfWidth
                              - s_iFlipHourglassWaistHalfWidth)
                   * wProfileQ16
                   + FLIP_Q16_HALF) >> 16);

    int32_t iCentreTwice = iOuterLeft + iOuterRight;
    *piLeft = (int16_t)((iCentreTwice - iAllowedHalfWidth + 1) >> 1);
    *piRight = (int16_t)((iCentreTwice + iAllowedHalfWidth) >> 1);
    return true;
#endif
}
                                        ...
                                        const flip_fluid_t *ptFluid)
{
    ...
    int32_t iMaximumHalfWidth = iContainerWidth - 1;
    int32_t iMaximumDistanceY = iContainerHeight - 1;
    s_iFlipHourglassMaximumHalfWidth = (int16_t)iMaximumHalfWidth;
    s_iFlipHourglassWaistHalfWidth = (int16_t)MAX(
        (iMaximumHalfWidth * FLIP_HOURGLASS_WAIST_PERCENT) / 100,
        2);
    s_wFlipHourglassProfileScaleQ16 = (iMaximumDistanceY > 0)
        ? (uint32_t)(((FLIP_HOURGLASS_PROFILE_SIZE - 1u) << 16)
                   / (uint32_t)iMaximumDistanceY)
        : 0u;
                                       ...
                                       uint32_t wFrameNr)
{
    ...
    if (flip_particle_uses_pre_neck_colour(ptFluid, iParticle)) {
        if (wLevel < 128u) {
            uint32_t wBlend = wLevel << 1;
            wRed = 72u + (wBlend * 160u) / 255u;
            wGreen = (wBlend * 64u) / 255u;
            wBlue = 8u - (wBlend * 4u) / 255u;
        } else {
            uint32_t wBlend = (wLevel - 128u) << 1;
            wRed = 232u + (wBlend * 23u) / 254u;
            wGreen = 64u + (wBlend * 172u) / 254u;
            wBlue = 4u + (wBlend * 44u) / 254u;
        }
...
static void __flip_update_particle_cache(flip_fluid_t *ptFluid)
{
    ...
    for (int16_t iParticle = 0;
        ...
        int16_t iWallLeft;
        int16_t iWallRight;
        if (__flip_get_hourglass_span(iY, &iWallLeft, &iWallRight)) {
            iX = __flip_clamp_i16(iX,
                                  (int16_t)(iWallLeft + 1),
                                  (int16_t)(iWallRight - 1));
        }
                                         ...
                                         uint8_t *pchBuffer)
{
    ...
    if (!__flip_get_hourglass_span(iY, &iLeft, &iRight)) {
        return;
    }
                                    ...
                                    uint_fast8_t chBitsPerPixel)
{
    ...
    for (int16_t iY = iYStart; iY < iYLimit; iY++) {
        ...
        if (NULL != ptFluid) {
            ...
            int16_t iWallLeft;
            int16_t iWallRight;
            bool bInsideHourglass = __flip_get_hourglass_span(
                                        iY,
                                        &iWallLeft,
                                        &iWallRight);
            ...
            if ((FLIP_SIM_RENDER_SURFACE == this.tRenderMode)
             ...
             && (iY >= iInnerTop) && (iY <= iInnerBottom)) {
                int16_t iDrawStart = MAX(iXStart,
                                         MAX(iInnerLeft, iWallLeft + 1));
                int16_t iDrawLimit = MIN(iXLimit,
                                         MIN(iInnerRight + 1, iWallRight));
                const flip_density_map_t *ptMapY =
